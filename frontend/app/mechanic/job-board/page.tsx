...
export default function MechanicJobBoardPage() {
  ...
  useEffect(() => {
    if (isLoaded) {
      try {
        localStorage.setItem("piveran_job_orders", JSON.stringify(jobOrders));
      } catch (e) {
        console.warn("localStorage quota exceeded, performing storage cleanup");
        try {
          localStorage.removeItem("piveran_job_orders_v1");
          localStorage.removeItem("piveran_job_orders_v2");
          localStorage.removeItem("piveran_job_orders_v3");
          localStorage.removeItem("piveran_job_orders_v4");
          localStorage.removeItem("piveran_job_orders_v9");
          localStorage.setItem("piveran_job_orders", JSON.stringify(jobOrders));
        } catch (err) {
              ...
              <div className="flex-1 overflow-y-auto">
                ...
                {drawerJobOrder.inspectionItems && drawerJobOrder.inspectionItems.length > 0 && (
                  <div className="px-5 py-4 border-b border-slate-100 space-y-3 text-xs">
                    ...
                    {drawerJobOrder.status !== "READY_FOR_PICKUP" && drawerJobOrder.status !== "COMPLETED" && drawerJobOrder.inspectionStarted !== false ? (
